...
import { makeDailyReportStateAdminPanelData, makeDailyReportStatePanelData } from './report';
...
export default ({data, loading})=>{
    ...
    const panelData=useMemo(()=>{
        ...
        if(all)return makeDailyReportStateAdminPanelData(send, receive);
        return makeDailyReportStatePanelData(send, receive);
    }, [data]);
                ...
                panelData.length>0 && panelData.map((aData, idx)=>(
                    <div style={{display:'flex', flexDirection:'row', flexWrap:'wrap', justifyContent:'center'}} key={idx}>                        
                        <Panel 
                            ...
                            panelStyle={{width:'calc(25% - 25px)', minWidth:180}}
